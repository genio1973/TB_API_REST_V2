...
class Team {
    ...
    function __construct($pdo, $id_team) {
        ...
        $this->teamInfo['matchs_perdus_0pts'] =  $this->teamInfo['matchs_perdus_1pt'] = $this->teamInfo['matchs_gagnes_2pts'] = $this->teamInfo['matchs_gagnes_3pts'] = 0; 
        $this->teamInfo['matchs_joues'] = 0;
        $this->teamInfo['sets_joues'] = 0;
        $this->teamInfo['sets_joues'] = 0;
        $this->teamInfo['matchs_joues'] = 0;
        $this->teamInfo['adversersaires_joues'] = NULL;
        ...
        if ($stmt->execute()){
            ...
            if($res){
                ...
                foreach($equipes_adverses_matchs_joues as $id_match => $id_adversaire){
                    $points = $this->matchResults($id_match);
                    $this->teamInfo['points_actuels'] += $points;
                    switch($points){
                        case 0 : $this->teamInfo['matchs_perdus_0pts'] ++; break;
     ...
     */
    private function matchResults($id_match){
    ...
    private function setsPointsWin(){
        ...
        if ($stmt->execute()){
            $res = $stmt->fetch(PDO::FETCH_ASSOC);
            $this->teamInfo['points_sets_realises'] = 0;
            $this->teamInfo['points_sets_encaisses'] = 0;
            if($res){
